...
description: "Practice an existing Learning Thread by applying saved understanding to one concrete task at a time."
disable-model-invocation: true
---

# Learning: Practice

Use this Skill only for explicit practice over an existing V6 Learning Thread.
...
- If no valid `thread.yaml` exists, say that durable practice material is unavailable
  and stop. Do not invent questions or create a thread.
- If the learner names a Question, practice that Question.
- Otherwise use `thread.current` and state which Question is being practiced.
...
Ordinary practice is read-only with respect to the Learning Thread:

- do not change `thread.root` or `thread.current`;
- do not add practice nodes or edges;
