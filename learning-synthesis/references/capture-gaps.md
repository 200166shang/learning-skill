...
Compile the accepted question into a delegation-ready ticket: a worker receiving only the ticket must be able to resolve it without the parent conversation. Keep lifecycle and routing identifiers in YAML frontmatter; put the actual work contract in the Markdown body: scope, exclusions, materials, evidence requirements, acceptance, delivery, and integration target. Compile every `code` ticket under the KnowledgeTicket `Code evidence rule`.
