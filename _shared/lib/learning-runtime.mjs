...
const files = ["workspace.yaml", "journey.yaml", "evidence.yaml", "state.yaml", "targets.yaml"];
...
function load(workspace, type) {
  ...
  if (inspection.status === "empty" && type === "start") return { journey: { version: 2, episodes: [], questions: [] }, evidence: emptyLearningEvidence(), state: idleLearningState(), targets: emptyLearningTargets() };
  if (inspection.status !== "current") throw new Error(`workspace is invalid for transition: ${inspection.status}`);
  return { journey: structuredClone(inspection.journey.journey), evidence: structuredClone(inspection.evidence.evidence), state: structuredClone(inspection.state.state), targets: structuredClone(inspection.targets.targets) };
}
...
function persistAtomically(workspace, snapshot) {
  ...
  try {
    writeWorkspaceManifest(stage); writeLearningJourney(stage, snapshot.journey); writeLearningTargets(stage, snapshot.targets, snapshot.journey); writeLearningEvidence(stage, snapshot.evidence, snapshot.journey, snapshot.targets); writeLearningState(stage, snapshot.state, snapshot.journey);
