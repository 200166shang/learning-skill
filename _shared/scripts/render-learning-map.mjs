...
function lineage(graph) {
  ...
  for (const edge of graph.edges.filter((item) => item.type === "journey")) {
  ...
  const visit = (id, depth, ancestry) => {
    ...
    const references = node.notePaths?.length ? ` ${node.notePaths.map((notePath) => `[知识笔记](${encodeURI(notePath)})`).join(" ")}` : "";
    lines.push(`${"  ".repeat(depth)}- ${label}${references}${suffix}`);
    rendered.add(id);
