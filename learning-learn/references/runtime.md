...
For ordinary teaching, use the document module's read-only `inspect` operation described in [document lifecycle](documents.md). It returns the current question and document context together. Cache the resolved source paths and command interface in the conversation; repeat inspection after a context loss, a conflict, or when current on-disk state is needed. Commands expose `--help` and schemas for discovery. Never infer an idle workspace from a command invoked on the wrong directory.
...
Ordinary document delivery uses one commit operation for chapter text and its Question association, rather than a Markdown edit followed by a separate reference mutation. Document transaction recovery is distinct from learning-schema migration: inspect the recovered text and continue the same teaching when recovery succeeds. Neither saving a document nor recovering it creates passing Evidence.
